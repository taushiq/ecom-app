...
select * from users;
select * from products p join categories c on c.id = p.cat_id;


select o.id, p.title as name, p.description, p.price, u.username from orders_details as od join orders as o
on od.order_id = o.id join products as p 
on od.product_id = p.id join users as u
on u.id = o.user_id;
